#include <starpu.h>
#include <iostream>
#include <omp.h>
#include <random>
#include <otx/otx.h>
#include <iostream>
#include <chrono>
#include <fstream>
#include <iostream>
#include <filesystem>
#include <vector>
#include <memory>
#include <mutex>
#include <unordered_map>
#include <algorithm>

using namespace std;
namespace fs = std::filesystem;

extern "C" {
#include <GraphBLAS.h>
}

#define TYPE_GrB GrB_BOOL
#define MAX_THREAD 64
#define MAX_BLOCKS_IN_OUTPUT 128*128 // 8x8: 1D=8x1, 2D=4x4
#include "../util/GrB_util.h"
#include "../util/get_feature.h"

#include "../util/block_matrix_feature.h"
#include "../util/timer.h"

#include "../scheduler/Scheduler.h"
#include "../util/reader_c.h"

#ifndef _USE_LIKWID
#define _USE_LIKWID
#endif

#ifdef _USE_LIKWID
#ifdef LIKWID_PERFMON
#include <likwid.h>
#else
#define LIKWID_MARKER_INIT
#define LIKWID_MARKER_THREADINIT
#define LIKWID_MARKER_SWITCH
#define LIKWID_MARKER_REGISTER(regionTag)
#define LIKWID_MARKER_START(regionTag)
#define LIKWID_MARKER_STOP(regionTag)
#define LIKWID_MARKER_CLOSE
#define LIKWID_MARKER_GET(regionTag, nevents, events, time, count)
#endif
#endif

struct TaskInfo {
    starpu_task *task;
    size_t i;
    size_t j;
    size_t k;
    double result;

    double mxmTime;

    TaskInfo(starpu_task *task, size_t i, size_t j, size_t k)
            : task(task), i(i), j(j), k(k), result() {}
};

struct TaskArgs {
    GrB_Matrix M;
    GrB_Matrix A;
    GrB_Matrix B;
    GrB_Matrix C;
    GrB_Descriptor descriptor;
    int nthreads;

    TaskInfo *taskInfo;
};


//auto tasks = createTasks(&cl, M, bA, B, M, false, true, nblocksI, nblocksJ, nblocksK, nthreads);
std::vector<TaskInfo *> createTasks(starpu_codelet *cl,  GrB_Matrix *bA, GrB_Matrix *B, GrB_Matrix *C,
                                    bool transposeA, bool transposeB, size_t nblocksM, size_t nblocksN,
                                    size_t nblocksL, int* nthreads) {
    std::vector<TaskInfo *> tasks;
    tasks.reserve(nblocksM * nblocksN * nblocksL);

    for (size_t i = 0; i < nblocksM; i++) {
        for (size_t j = 0; j < nblocksN; j++) {
            for (size_t k = 0; k < nblocksL; k++) {
                size_t idxC = i * nblocksN + j;
                size_t idxA = i * nblocksL + k;;
                size_t idxB = j * nblocksL + k;

                auto task = starpu_task_create();
                task->cl = cl;

                auto args = static_cast<TaskArgs *>(malloc(sizeof(TaskArgs)));
                args->A = bA[idxA];
                args->B = B[0];
                args->C = C[0];

                GrB_Descriptor desc;
                OK(GrB_Descriptor_new(&desc));
                OK(GxB_Desc_set(desc, GrB_OUTP, GrB_REPLACE));
                if (transposeA) { OK(GxB_Desc_set(desc, GrB_INP0, GrB_TRAN)); }
                if (transposeB) { OK(GxB_Desc_set(desc, GrB_INP1, GrB_TRAN)); }
                OK(GxB_Desc_set(desc, GrB_MASK,  GrB_COMP));
                OK(GxB_Desc_set(desc, GxB_DESCRIPTOR_NTHREADS, 1));
                args->descriptor = desc;
                args->nthreads = nthreads[idxC];
                //cout << "bid=" << idxC << ", nthreads=" << nthreads[idxC] << endl;
                //cout.flush();
                task->cl_arg = args;
                task->cl_arg_size = sizeof(TaskArgs);
                task->cl_arg_free = 1;

                auto taskInfo = new TaskInfo{task, i, j, k};
                args->taskInfo = taskInfo;

                tasks.push_back(taskInfo);
            }
        }
    }

    return tasks;
}

void test_scheduler_spgemm(string name, GrB_Matrix A, GrB_Matrix B,GrB_Matrix M,
                           size_t nblocksI, size_t nblocksJ, size_t nblocksL,
                           size_t niter, size_t skip, double* featuresA, double* featuresB, double* featuresC, double* featuresM,
                           vector<double> &report_time,
                           double* init_time,
                           Scheduler *scheduler,
                           int runscheduler, int rungrb, int runomp,
                           bool transposeA, bool transposeB) {

    GrB_Matrix *bA;

    GrB_Index nrowsA, ncolsA, nrowsB, ncolsB;
    GrB_Matrix_nrows(&nrowsA, A);
    GrB_Matrix_ncols(&ncolsA, A);
    GrB_Matrix_nrows(&nrowsB, B);
    GrB_Matrix_ncols(&ncolsB, B);


    size_t nrows_block_A, ncols_block_A;
    nrows_block_A = (nrowsA + nblocksI - 1) / nblocksI;
    ncols_block_A = ncolsA; //(nrowsB + nblocksJ - 1) / nblocksJ;

    size_t nrows_blocked_A, ncols_blocked_A;
    create_blocks_type(&bA, &nrows_blocked_A, &ncols_blocked_A, A, nrows_block_A, ncols_block_A, false, GrB_FP64,featuresM);
    //GrB_Matrix *bC;
    //size_t ncols_blocked_C, nrows_blocked_C;
    //create_blocks_type(&bC, &nrows_blocked_C, &ncols_blocked_C, A, nrows_block_A * nrowsB, ncols_block_A * ncolsB,false, GrB_FP64,featuresC);

    double result = 0;
    // region init codelet
    starpu_codelet cl{};
    starpu_codelet_init(&cl);
    cl.cpu_funcs[0] = [](void *buffers[], void *clArgs) {
        auto args = static_cast<TaskArgs *>(clArgs);
        OK(GxB_Desc_set(args->descriptor, GxB_DESCRIPTOR_NTHREADS, args->nthreads));
        //GrB_Matrix block_A = args->A;
        GrB_Index nrows_A_block;
        GrB_Index ncols_A_block;
        GrB_Index nrowsB;
        GrB_Index ncolsB;
        GrB_Matrix_nrows(&nrows_A_block, args->A);
        GrB_Matrix_ncols(&ncols_A_block, args->A);
        GrB_Matrix_nrows(&nrowsB, args->B);
        GrB_Matrix_ncols(&ncolsB, args->B);
        GrB_Matrix block_C;
        OK(GrB_Matrix_new(&block_C, GrB_FP64, nrows_A_block * nrowsB, ncols_A_block * ncolsB));
        //printf("nrows_A_block=%lu,ncols_A_block=%lu,nrows_B_block=%lu,ncols_B_block=%lu,Crow=%lu, Ccol=%lu\n",
        //       nrows_A_block,ncols_A_block,
        //       nrowsB,ncolsB,
        //       nrows_A_block * nrowsB, ncols_A_block * ncolsB);
        //double result = 0;
        OK(GrB_Matrix_kronecker_BinaryOp(block_C, GrB_NULL, GrB_NULL, GrB_TIMES_FP64, args->A, args->B, GrB_NULL));
        //OK(GrB_Matrix_reduce_FP64(&result, GrB_PLUS_FP64, GrB_PLUS_MONOID_FP64, block_C, GrB_NULL));
        ////OK(GrB_Matrix_kronecker_BinaryOp(args->C, GrB_NULL, GrB_NULL, GrB_TIMES_FP64, args->A, args->B, GrB_NULL));
        ////GrB_Matrix_reduce_FP64(&result, GrB_PLUS_FP64, GrB_PLUS_MONOID_FP64, args->C, GrB_NULL);
        //GrB_Matrix_free(&block_C);
        //args->taskInfo->result = result;
        //printf("LOG;%lf\n", result);
        //args->taskInfo->mxmTime = timer_end();
    };
    cl.nbuffers = 0;

    int myworker = 0;
    /*
    int nthreads[4]={16,1,0,16};
    vector<int> order(4, 0);
    order[0]=0;
    order[1]=3;
    order[2]=999;
    order[3]=1;
     */
    int block_id,block_thread;
    vector<int> order;
    int nthreads[16384]={0};
    string a="new_kron_spgemm_mask_comp_"+name+".csv";
    //cout << a  << "," << nblocksM << "," << nblocksN << "," << nblocksL << endl;
    //cout.flush();
    FILE* fp = fopen(a.c_str(),"r");
    while (fscanf(fp, "%d,%d", &block_id, &block_thread ) == 2) {
        //cout << block_id << "," << block_thread << endl;
        //cout.flush();
        order.push_back(block_id);
        if (block_id == 99999) {
            //printf("It's ok, wait...\n");
            //fflush(stdout);
            continue;
        }
        nthreads[block_id]=block_thread;
        //printf("%d,%d\n",block_id, nthreads[block_id]);
        //fflush(stdout);
    }

    fclose(fp);
    if ((nblocksI==1) && (nthreads[0]==MAX_THREAD)){
        cout << name << " uses the same config with baseline." << endl;
        cout.flush();
        return ;
    }


    //printf("myblock_id.size=%d\n",order.size());
    //fflush(stdout);
    if (nthreads[order[0]]< scheduler->getNumWorkers() ) {
        int ii=0;
        scheduler->splitWorker(ii * nthreads[order[0]], scheduler->getNumWorkers() / nthreads[order[0]], nthreads[order[0]]);
        //printf("1-splitWorker %d into %d Q with %d threads\n", ii, scheduler->getNumWorkers()/ nthreads[order[0]],
        //       nthreads[order[0]]);
        //fflush(stdout);
    }

#ifdef _USE_LIKWID
    LIKWID_MARKER_START("scheduler");
#endif
    double min_time=9999;
    for (int iter = 0; iter < niter + skip; iter++) {
        //cout << "create tasks, niter=" << niter <<  endl;
        //cout.flush();
        auto tasks = createTasks(&cl, bA, &B, bA, false, true, nblocksI, nblocksJ, nblocksL, nthreads);
        if (iter>0) scheduler->mergeWorkers(0, MAX_THREAD);
        if (nthreads[order[0]] != MAX_THREAD) {
            int split_iter=scheduler->getNumWorkers()/MAX_THREAD;
            for (int ii = 0; ii < split_iter; ii++) {
                scheduler->splitWorker(ii * MAX_THREAD, MAX_THREAD / nthreads[order[0]], nthreads[order[0]]);
                //printf("splitWorker %d into %d Q with %d threads\n", ii, MAX_THREAD / nthreads[order[0]], nthreads[order[0]]);
                //fflush(stdout);
            }
        }
        scheduler->waitForAll();
        if (iter >= skip) timer_start();
        //cout << "End create tasks" << endl;
        //cout.flush();
        int nextWorker = 0;
        int old_threads= nthreads[order[0]];
        for (int taskid=0; taskid<order.size();taskid++) {

            //cout << "blockTask:" << taskid << "/" << order.size() << "," << order[taskid]<< endl;
            //cout.flush();
            if (order[taskid] == 99999) {
                //cout << "wait for all" << endl;
                //cout.flush();
                scheduler->waitForAll();
                int split_iter=scheduler->getNumWorkers()/old_threads;
                for (int ii = 0; ii < split_iter; ii++) {
                    scheduler->splitWorker(ii * old_threads, old_threads / nthreads[order[taskid+1]], nthreads[order[taskid+1]]);
                    //printf("2-splitWorker %d into %d Q with %d threads\n", ii, old_threads / nthreads[order[taskid+1]], nthreads[order[taskid+1]]);
                    //fflush(stdout);
                }
                nextWorker=0;
            }else{
                scheduler->submitTask(tasks[order[taskid]]->task, nextWorker * (nthreads[order[taskid]]), nthreads[order[taskid]]);
                //cout << "iter=" << iter << ", submit task to Q "<< nextWorker* (nthreads[order[taskid]]) << ", nthreads=" << nthreads[order[taskid]] << endl;
                //cout.flush();
                old_threads=nthreads[order[taskid]];
                nextWorker++;
                if (nextWorker >= MAX_THREAD/old_threads) nextWorker=0;
            }

        }
        scheduler->waitForAll();
        if (iter >= skip) {
            report_time[0] += timer_end();
            cout << "iter," <<iter <<  "," <<  report_time[0]/(iter-skip+1) << endl;
            cout.flush();


            //report_time[0] = timer_end();
            //if (report_time[0] < min_time) min_time=report_time[0];
            //cout << "spu = " << min_time  << ", iter=" << iter << endl;
            //cout.flush();
        }

    }
    cout << "spu," << report_time[0]/niter << endl;
    cout.flush();
    //cout << "spu = " << min_time   << endl;
    //cout.flush();
#ifdef _USE_LIKWID
    LIKWID_MARKER_STOP("scheduler");
    LIKWID_MARKER_CLOSE;
#endif
    free_blocks(bA, nrows_blocked_A, ncols_blocked_A);

}

void test_grbbase_spgemm(string name, GrB_Matrix A, GrB_Matrix B, GrB_Matrix M,
                         size_t nblocksI, size_t nblocksJ, size_t nblocksK,
                         size_t niter, size_t skip, double* featuresA, double* featuresB, double* featuresC, double* featuresM,
                         vector<double> &report_time,
                         double* init_time,
                         Scheduler *scheduler,
                         int runscheduler, int rungrb, int runomp,
                         bool transposeA, bool transposeB){

    //cout << "Enter...baseline," << name << endl;
    //cout.flush();
    GrB_Matrix *bA;
    GrB_Index nrowsA, ncolsA, nrowsB, ncolsB;
    GrB_Matrix_nrows(&nrowsA, A);
    GrB_Matrix_ncols(&ncolsA, A);
    GrB_Matrix_nrows(&nrowsB, B);
    GrB_Matrix_ncols(&ncolsB, B);


    size_t nrows_block_A, ncols_block_A;
    nrows_block_A = (nrowsA + nblocksI - 1) / nblocksI;
    ncols_block_A = (nrowsB + nblocksJ - 1) / nblocksJ;


    size_t nrows_blocked_A, ncols_blocked_A;
    create_blocks_type(&bA, &nrows_blocked_A, &ncols_blocked_A, A, nrows_block_A, ncols_block_A, false, GrB_FP64,featuresM);


    // region init codelet
    starpu_codelet cl{};
    starpu_codelet_init(&cl);
    cl.cpu_funcs[0] = [](void *buffers[], void *clArgs) {
        auto args = static_cast<TaskArgs *>(clArgs);
        OK(GxB_Desc_set(args->descriptor, GxB_DESCRIPTOR_NTHREADS, args->nthreads));
        //GrB_Matrix block_A = args->A;
        GrB_Index nrows_A_block;
        GrB_Index ncols_A_block;
        GrB_Index nrowsB;
        GrB_Index ncolsB;
        GrB_Matrix_nrows(&nrows_A_block, args->A);
        GrB_Matrix_ncols(&ncols_A_block, args->A);
        GrB_Matrix_nrows(&nrowsB, args->B);
        GrB_Matrix_ncols(&ncolsB, args->B);
        double result = 0;
        GrB_Matrix block_C;
        OK(GrB_Matrix_new(&block_C, GrB_FP64, nrows_A_block * nrowsB, ncols_A_block * ncolsB));
        //printf("nrows_A_block=%lu,ncols_A_block=%lu,nnzA=%lf\nnrows_B_block=%lu,ncols_B_block=%lu\nCrow=%lu, Ccol=%lu\n",
        //       nrows_A_block,ncols_A_block,featuresM[3],
        //       nrowsB,ncolsB,
        //       nrows_A_block * nrowsB, ncols_A_block * ncolsB);
        OK(GrB_Matrix_kronecker_BinaryOp(block_C, GrB_NULL, GrB_NULL, GrB_TIMES_FP64, args->A, args->B, GrB_NULL));
        GrB_Matrix_reduce_FP64(&result, GrB_PLUS_FP64, GrB_PLUS_MONOID_FP64, block_C, GrB_NULL);
        GrB_Matrix_free(&block_C);
        args->taskInfo->result = result;
        //printf("LOG;%lf\n", result);
    };
    cl.nbuffers = 0;

    //cout << "End codelet" << endl;
    //cout.flush();

    int myworker = 0;
    int nthreads[1]={MAX_THREAD};
    vector<int> order(1, 0);
    order[0]=0;

#ifdef _USE_LIKWID
    LIKWID_MARKER_START("grb");
#endif
    //cout << "Before loop," << niter << "," << skip << endl;
    //cout.flush();
    double min_time=999;
    for (int iter = 0; iter < niter + skip; iter++) {
        if (iter >= skip) timer_start();
        //cout << "before create tasks" << endl;
        //cout.flush();
        auto tasks = createTasks(&cl, bA, &B, &M, false, true, nblocksI, nblocksJ, nblocksK, nthreads);
        //cout << "end create tasks" << endl;
        //cout.flush();
        scheduler->submitTask(tasks[0]->task, 0, MAX_THREAD);
        //cout << "iter=" << iter << ", submit task to Q0, nthreads=" << MAX_THREAD << endl;
        //cout.flush();
        scheduler->waitForAll();
        if (iter >= skip)  {
            report_time[1] += timer_end();
            cout << "iter," <<iter <<  "," <<  report_time[1]/(iter-skip+1) << endl;
            cout.flush();
        }
    }
    //scheduler->waitForAll();
    //report_time[0] += timer_end();
    cout << "base," << report_time[1]/niter << endl;
    cout.flush();

#ifdef _USE_LIKWID
    LIKWID_MARKER_STOP("grb");
    LIKWID_MARKER_CLOSE;
#endif
    //freeMxMBlocks(nullptr,bM, bA, bB, nblocksM, nblocksN, nblocksL);
    //printf("Done grb\n");
    //fflush(stdout);
}

double init_time[15]={0.0};
int main(int argc, char *argv[]) {
    auto niter = otx::argTo<size_t>(argc, argv, {"-niter"}, 1);
    auto nskip = otx::argTo<size_t>(argc, argv, {"-nskip"}, 1);
    auto opt = otx::argTo<int>(argc, argv, "-o", 0);
    auto runscheduler = otx::argTo<int>(argc, argv, {"-runscheduler"}, 0);
    auto rungrb = otx::argTo<int>(argc, argv, {"-rungrb"}, 0);
    auto pathA = otx::argTo<std::string>(argc, argv, "-A", INPUTS_DIR"/simple");
#ifdef _USE_LIKWID
    LIKWID_MARKER_INIT;

#pragma omp parallel
    {
        //LIKWID_MARKER_THREADINIT;
        LIKWID_MARKER_REGISTER("grb");
        LIKWID_MARKER_REGISTER("scheduler");
    }
#endif


    OK(GrB_init(GrB_NONBLOCKING))


    int runomp = 0;
    Scheduler scheduler;
    auto numWorkers = scheduler.getNumWorkers();

    cout << "numWorkers = " << numWorkers << endl;
    cout.flush();

    if (numWorkers == 128) {
        scheduler.start({128, 64, 32, 16, 8, 4, 1});
    } else if (numWorkers == 64) {
        scheduler.start({64, 32, 16, 8, 1});
    } else if (numWorkers == 32) {
        scheduler.start({32, 16, 8, 4, 1});
    } else {
        cout << "numWorkers = " << numWorkers << endl;
        cout.flush();
    }

    int new_matrix = 0;
    int model_nthreads;

    //for(auto& p: fs::recursive_directory_iterator("/home/nanding/train_matrix/")) {
    //for(auto& p: fs::recursive_directory_iterator("/home/nanding/big_matrix/")) {
    //for(auto& p: fs::recursive_directory_iterator("/home/nanding/refer_matrix/")) {
    //for(auto& p: fs::recursive_directory_iterator("/project/projectdirs/m2956/nanding/myprojects/dyfuse/matrix/profile_matrix")) {
    //for(auto& p: fs::recursive_directory_iterator("/project/projectdirs/m2956/nanding/myprojects/dyfuse/matrix/srdan_matrix")){
    //for(auto& p: fs::recursive_directory_iterator("/home/nanding/test_matrix_quick/")) {
    int nblocks = 1;
    string a = pathA.c_str();
    std::replace(a.begin(), a.end(), '/', ' ');
    stringstream ss(a);
    string word;
    while (ss >> word) {};
    if (runscheduler==1){

        /* try to open file to read */
        string name = "new_kron_spgemm_mask_comp_block_" + word + ".csv";
        ifstream ifile;
        ifile.open(name);
        if (ifile) {
            cout << "file exists" << endl;

            int r, c;
            FILE *fp = fopen(name.c_str(), "r");

            while (fscanf(fp, "%d,%d", &r, &c) == 2) {
                nblocks = c;
            }

            fclose(fp);
            cout << "Testing:" << word << "," << nblocks << '\n';
            cout.flush();

        } else {
            cout << name << " file doesn't exist" << endl;
            return 0;
        }
    }

    GrB_Matrix A;
    GrB_Matrix B;
    GrB_Matrix M;

    timer_start();
    if (read_Matrix_FP64(&A, pathA.c_str(), 0) != GrB_SUCCESS) { return 1; }
    init_time[0] += timer_end();
    //cast_matrix(&A, GrB_UINT64, GxB_ONE_UINT64);
    OK(GrB_Matrix_dup(&B, A));
    OK(GrB_Matrix_dup(&M, A));
    //cout << "end read matrix," << pathA.c_str() << endl;
    //cout.flush();
    // 1D
    if (rungrb==1) {
        double *featuresA,*featuresB,*featuresC, *featuresM;
        if (!(featuresA = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresA[].");
        if (!(featuresB = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresB[].");
        if (!(featuresC = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresC[].");
        if (!(featuresM = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresM[].");
        int nblocks=1;
        if (!(featuresM = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresM[].");
        vector<double> report_time(MAX_BLOCKS_IN_OUTPUT, 0);
        test_grbbase_spgemm(word, A,B,M, nblocks, 1, 1,
                            niter,nskip, featuresA, featuresB,featuresC,featuresM,
                            report_time,
                            init_time,
                            &scheduler, runscheduler, rungrb, runomp,
                            false, true);
        free(featuresA);
        free(featuresB);
        free(featuresM);
    }

    if (runscheduler==1) {
        //cout << "nblocks=" << nblocks << endl;
        //cout.flush();
        double *featuresA,*featuresB,*featuresC, *featuresM;
        if (!(featuresA = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresA[].");
        if (!(featuresB = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresB[].");
        if (!(featuresC = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresC[].");
        if (!(featuresM = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT("Malloc fails for featuresM[].");
        if (!(featuresM = (double *) malloc(nblocks * nblocks * NUM_FEATURE * sizeof(double))))
        ABORT(
                "Malloc fails for featuresM[].");
        vector<double> report_time(MAX_BLOCKS_IN_OUTPUT, 0);
        // 2D
        test_scheduler_spgemm(word,A,B,M, nblocks, nblocks, 1,
                              niter, nskip,featuresA, featuresB,featuresC,featuresM,
                              report_time,
                              init_time,
                              &scheduler, runscheduler, rungrb, runomp,
                              false, true
        );
        // 1D
        //test_scheduler_spgemm(word,A,B,M, nblocks, 1, 1,
        //                      niter, nskip,featuresA, featuresB,featuresC,featuresM,
        //                      report_time,
        //                      init_time,
        //                      &scheduler, runscheduler, rungrb, runomp,
        //                      false, true
        //);

        //myfile << "omp," << p.path().c_str() << "," << nblocks << "," << gbase_time[nblocks][0]/niter << endl;
        //myfile.flush();
        free(featuresA);
        free(featuresB);
        free(featuresM);
    }



    GrB_Matrix_free(&A);
    GrB_Matrix_free(&B);


    //}

    scheduler.stop();
#ifdef _USE_LIKWID
    LIKWID_MARKER_CLOSE;
#endif
    OK(GrB_finalize());
    return 0;
}
