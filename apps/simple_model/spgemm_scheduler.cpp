...
void test_scheduler_spgemm(string name, GrB_Matrix A, GrB_Matrix B,GrB_Matrix M,
    ...
    string a="new_spgemm_mask_"+name+".csv";
...
void test_grbbase_spgemm(string name, GrB_Matrix A, GrB_Matrix B, GrB_Matrix M,
    ...
    GrB_Matrix_new(&C, GrB_UINT64, m, n);
    create_blocks(&bC, &bm, &bn, C, block_size_m, block_size_n, 0);
    //cout << "End create blocks," << nblocksM << endl;
    //cout.flush();
    ...
    scheduler->waitForAll();
    report_time[0] += timer_end();
    cout << "base, iter=" << niter << "," << report_time[0]/niter << endl;
    cout.flush();
...
#pragma omp parallel
    ...
    if (runscheduler==1){
        ...
        string name = "new_spgemm_mask_block_" + word + ".csv";
        ifstream ifile;
        ifile.open(name);
        if (ifile) {
    ...
    if (rungrb==1) {
        ...
        test_grbbase_spgemm(word, A,B,M, nblocks, nblocks, 1,
                            niter, featuresA, featuresB,featuresC,featuresM,
                            report_time,
                            init_time,
