...
void test_scheduler_spgemm(const char *name, GrB_Matrix A, GrB_Matrix B, GrB_Matrix M,
                    ...
                    for (size_t k = 0; k < nblocksL; k++) {
                        ...
                        for (int iter = 0; iter < niter + skip; iter++) {
                            ...
                            OK(GrB_mxm(bC[i * nblocksN + j], bM[i * nblocksN + j], GrB_NULL, GrB_PLUS_TIMES_SEMIRING_UINT64,
                                       bA[i * nblocksL + k], bB[j * nblocksL + k], GrB_DESC_RCT1));
                            //OK(GrB_mxm(bC[i * nblocksN + j], bM[i * nblocksN + j], GrB_NULL, GrB_PLUS_TIMES_SEMIRING_UINT64,
    ...
    std::string rankfile("spgemm_maskA_comp_train_");
...
int main(int argc, char *argv[]) {
    ...
    std::string rankfile("spgemm_maskA_comp_train_0");
    ...
    while (flag) {
        ...
        if (ifile) {
            ...
            ifile.close();
            rankfile = "spgemm_maskA_comp_train_";
    ...
    //for(auto& p: fs::recursive_directory_iterator("/home/nanding/test_matrix_quick/")) {
        ...
        int nblocks=1, runscheduler=1, rungrb=1;
        while (nblocks<=2) {
        ...
        strcat(cmd," ");
        strcat(cmd, "/home/nanding/spgemm_maskA_finish_train_matrix/");
