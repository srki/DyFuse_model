...
void mxm_func(void *buffers[], void *clArgs) {
    ...
    double time;
    int movedPreMxM = 0, movedPostMxM = 0;
...
#pragma omp parallel num_threads(args->block_parallelism) default(none) shared(myId) reduction(+:movedPreMxM)
    {
        int cpuid = sched_getcpu();
        if (cpuid !=  myId + omp_get_thread_num()) {
            movedPreMxM++;
        }
...
#pragma omp parallel num_threads(args->block_parallelism) default(none) shared(myId) reduction(+:movedPostMxM)
    {
        if (sched_getcpu() !=  myId + omp_get_thread_num()) {
            movedPostMxM++;
        }
    }
...
#if defined(HAVE_EXEC_INFO_SUPPORT)
    printf("BLOCK;%zu;%zu;%d;%d;%lf;", args->i,  args->j, movedPreMxM, movedPostMxM, time);
