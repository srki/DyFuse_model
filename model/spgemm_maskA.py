...
if time_regression==1:
    for q in range (1,qnum):
        ...
        #    r2_value3=float(-1.0)
        best=max(r2_value1,r2_value2)
        #best=max(r2_value1,r2_value2,r2_value3)
        print("class ",q," nnz range:",min(nnzs),max(nnzs),'sample:',len(nnzs),'r2 score:', r2_value1,r2_value2);
        ...
        if best == r2_value1 and flag==0:
            ...
            del_model_name1="kernel/"+machine+"/linear2_1D_thres_"+str(thres)+"_class_"+str(threads_num_in_q[q])+"_train_data_"+apps+".csv"+".pkl"
    ...
    with open(predict_filename,'r') as csvfile:
        ...
        for row in predict_filerow:
            if len(row) == 10:
                ...
                #if flag == 0:
                #    log_name="test/"+machine+"/"+apps+"_"+row[0]+".csv"
                #    if os.path.exists(log_name):
                #        has_head=1
                #    else:
...
#
    ...
    blocks2D=[] #[8,27,64]
    #matrix_name=["roadNet-TX.mtx"]
    for name in matrix_name:
        filename="test/"+machine+"/"+apps+"_"+name+".csv"
        print("processing...",name,filename)
        if not os.path.exists(filename):
            print("can't find ",filename)
            #filename="test/"+machine+"/kron_"+name+".csv"
        ...
        #print("#predict_threads:",len(predict_threads))
        ...
        for i in range(0,len(elem)):
            ...
            if len(raw_test_data[:,0])+1 > np.sum(elem[0:i+1]):
                old=np.sum(elem[0:i+1])
                #print("continue, old=",old)
                continue
            elif len(raw_test_data[:,0])+1 == np.sum(elem[0:i+1]):
                confignum=i+1
            elif (len(raw_test_data[:,0]) +1 < np.sum(elem[0:i+1])) and (len(raw_test_data[:,0])+1 > old):
        ...
        if time_predict==1:
            ...
            if 'linear2' in model:
                ...
                for my_indexes in indexes:
                    ...
                    #print("linear2...",my_indexes,modeled_time[my_indexes])
            if 'linear3' in model:
                ...
                for my_indexes in indexes:
                    ...
                    #print("linear3...",my_indexes,modeled_time[my_indexes])
            ...
            for x in predict_threads:
                ...
                if x == 0:
                    ...
                    #print(len(predict_threads),"In time predict, x=",x,",myclass=",myclass[0],',i=',i,"time=",modeled_time[myclass[0]-1][i])
        ...
        if scheduler==1:
            ...
            with open('res/res_kron','a') as csvfile:
                writer = csv.writer(csvfile, delimiter=',')
                writer.writerow([name,"Model",config_name[best_config]])

            res_file="res/"+machine+"/new_kron_"+apps+"_"+name+".csv"
            res_file1="res/"+machine+"/new_kron_"+apps+"_block_"+name+".csv"
