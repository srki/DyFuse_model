/* Copied from http://people.eecs.berkeley.edu/~aydin/GraphBLAS_API_C_v13.pdf */
...
GrB_Info BC_update(GrB_Vector *delta, GrB_Matrix A, GrB_Index *s, GrB_Index nsver) {
    ...
    GrB_Index nvasls = 0;   // nvals == 0 when BFS phase is complete
    ...
    do {
        ...
        GrB_mxm(frontier, numsp, GrB_NULL, GxB_PLUS_TIMES_INT32, A, frontier, GrB_DESC_RCT0); // f<!<numsp> = A' +.* f (update frontier_
        GrB_Matrix_nvals(&nvasls, frontier); // number of nodes in frontier at this level
        d++;
    } while (nvasls);
    ...
    for (int i = d - 1; i > 0; i--) {
        ...
        // add contributions by successors and mask with that BGS level's frontier
        GrB_mxm(w, sigmas[i - 1], GrB_NULL, GxB_PLUS_TIMES_FP32, A, w, GrB_DESC_R); //w<sigmas[i-1]> = (A +.* w)
