...
GrB_Info create_blocks_type(GrB_Matrix **bA_out, GrB_Index *nrows_blocked_out, GrB_Index *ncols_blocked_out,
                            GrB_Matrix A, size_t nrows_block, size_t ncols_block, bool full_idxes, GrB_Type type) {
    if (type == GrB_UINT64) {
        return create_blocks_uint64(bA_out, nrows_blocked_out, ncols_blocked_out, A, nrows_block, ncols_block, full_idxes);
    } else if (type == GrB_FP64) {
        return create_blocks_double(bA_out, nrows_blocked_out, ncols_blocked_out, A, nrows_block, ncols_block, full_idxes);
    } else {
...
GrB_Info create_blocks_features(GrB_Matrix **bA_out, GrB_Index *nrows_blocked_out, GrB_Index *ncols_blocked_out,
    ...
    for (size_t bi = 0; bi < nrows_blocked; bi++) {
        for (size_t bj = 0; bj < ncols_blocked; bj++) {
            ...
            //printf("idx=%ld, nrows=%ld, ncols=%ld, nnz=%ld\n",block_idx, nrows_block, ncols_block, nzb_cnt[block_idx]);
            //fflush(stdout);
        }
    }
    //for (size_t bi = 0; bi < nrows_blocked; bi++) {
